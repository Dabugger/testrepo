<script lang="ts">
	...
	import Modal from '$lib/components/CreateSimpleRouteModal.svelte';
	import { createMenubar, melt } from '@melt-ui/svelte';
	import { writable } from 'svelte/store';
	import { ChevronRight, Check } from 'lucide-svelte';
	import SimpleRouteModal from '$lib/components/SimpleRouteModal.svelte'
	...
	function startNewCalc() {
		console.log('open dialog start new route');
	...
	function initiateSimpleRouteCalculation() {
		//add this route to a temporary route collection
		console.log('INIT');
		const dialog = document.getElementById('dialog');
		dialog.classList.toggle('hidden');
	}
...
<div class="flex rounded-md bg-white p-1 shadow-md" use:melt={$menubar}>
	...
	<div class="menu" use:melt={$menu}>
		<div class="item" use:melt={$item} on:click={startNewCalc}>Nueva Ruta Simple</div>
